...
const ReportDiscount = () => {
    ...
    useEffect(() => {
        let active = true;
        DiscountService.fetchDiscountReport(initialDates)
    ...
    const generate = async (event) => {
        ...
        setLoading(true);
        try {
            const { data } = await DiscountService.fetchDiscountReport(dates);
            setReport(data || { data: [] });
    ...
    return (
        <main className="discount-loss-report discount-report" aria-busy={loading}>
            <header className="dlr-header">
                <div><p className="dlr-eyebrow">Reports / Discounts</p><h1>Discount report</h1>
                    <p>Review discounted sales, item pricing, and total discounts.</p></div>
            ...
            <section className="dlr-details" aria-labelledby="discount-items-title">
                <div className="dlr-table-heading"><div><h2 id="discount-items-title">Discounted items</h2><p>Discount amounts first, with per-item pricing and order totals for context.</p></div><span>{rows.length} items</span></div>
                ...
                <div className="dlr-table-scroll" tabIndex="0" role="region" aria-label="Discounted items table">
                    <table>
                        ...
                        {!loading && !error && rows.length === 0 && <tr><td colSpan="7" className="dlr-empty"><strong>No discounted items found</strong><span>Try another date range to see more results.</span></td></tr>}
                        </tbody>
                    </table>
                </div>
                <p className="dlr-footnote">Price / item shows the original price → current order price per item. Total discount = discount per item × quantity. Order totals and total discounts cover all units in the row. — means the price is unavailable.</p>
            </section>
        </main>
    );
};
