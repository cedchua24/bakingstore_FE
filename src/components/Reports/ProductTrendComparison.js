    ...
    return <main className="pr-page pt-page">
        ...
        <section className="pr-card"><header><div><h2>Monthly product movement</h2><p>{filtered.length} products · {monthLabels.length ? monthLabels.join(' vs ') : 'three-month comparison'}</p></div><TextField className="pr-search" size="small" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search product..." InputProps={{ startAdornment: <InputAdornment position="start"><SearchRoundedIcon/></InputAdornment> }}/></header><div className="table-responsive"><table className="pr-table pt-table"><thead><tr><th>Rank movement<span className="pt-th-hint">Current vs previous</span></th><th>Product</th><th>Current month<span className="pt-th-hint">Sales and quantity</span></th><th>Previous months<span className="pt-th-hint">Sales and quantity</span></th><th>Current stock</th><th>Sales & quantity trend<span className="pt-th-hint">Changes and month-end forecast</span></th><th className="pt-graph-heading" title="View graph"><BarChartRoundedIcon fontSize="small"/></th></tr></thead><tbody>
            {filtered.map((item, index) => {
                ...
                const previousMonth = new Date(`${filters.month}-01T00:00:00`);
                previousMonth.setMonth(previousMonth.getMonth() - 1);
                const previousMonthKey = `${previousMonth.getFullYear()}-${String(previousMonth.getMonth() + 1).padStart(2, '0')}`;
                const previousMonthHistory = historyRows.find(entry => entry.month === previousMonthKey);
                const previousMonthSales = previousMonthHistory ? historySales(previousMonthHistory) : comparisonSales(item).previous;
                const reportedAverageSales = pick(item, ['average_sales', 'three_month_average_sales', 'average_sales_amount'], NaN);
                const averageSales = finite(reportedAverageSales) ? Number(reportedAverageSales) : historyRows.length ? historyRows.reduce((sum, entry) => sum + historySales(entry), 0) / historyRows.length : null;
                const previousSalesChange = currentMonthSales - previousMonthSales;
                const averageSalesChange = averageSales === null ? null : currentMonthSales - averageSales;
                ...
                return <tr key={item.product_id || item.mark_up_product_id || item.id || index}><td><div className="pt-rank-move"><span className="pr-rank">{currentRank}</span><div><small>was #{previousRank}</small><strong className={rankChange < 0 ? 'pr-negative' : 'pt-positive'}>{rankChange > 0 ? '▲ ' : rankChange < 0 ? '▼ ' : ''}{Math.abs(rankChange) || '—'}</strong></div></div></td><td><div className="pr-product"><strong>{name}</strong><i className={`pt-product-verdict ${rankChange > 0 ? 'pt-product-verdict--up' : rankChange < 0 ? 'pt-product-verdict--down' : 'pt-product-verdict--same'}`} title={rankChange > 0 ? `Rank improved by ${rankChange}` : rankChange < 0 ? `Rank dropped by ${Math.abs(rankChange)}` : 'Rank unchanged'} aria-label={rankChange > 0 ? `Rank improved by ${rankChange}` : rankChange < 0 ? `Rank dropped by ${Math.abs(rankChange)}` : 'Rank unchanged'}/><span>#{item.product_id || item.id || '—'} · {piecesPerPackage} Pc/{packaging}</span></div></td><td><div className={`pt-current-month ${trend.vsPrevious > 0 ? 'pt-current-month--up' : trend.vsPrevious < 0 ? 'pt-current-month--down' : 'pt-current-month--same'}`}><small>{currentHistory?.month || filters.month}</small><strong>{money(currentMonthSales)}</strong><em title={soldQuantityTitle(currentMonthQuantity, piecesPerPackage, packaging)}>{soldQuantityLabel(currentMonthQuantity, piecesPerPackage, packaging)}</em></div></td><td><div className="pt-history">{previousHistory.map((entry, i) => { const entryQuantity = soldQuantity(entry); return <span key={entry.month || i}><small>{entry.month || entry.label || `M${i + 1}`}</small><strong>{money(historySales(entry))}</strong><em title={soldQuantityTitle(entryQuantity, piecesPerPackage, packaging)}>{soldQuantityLabel(entryQuantity, piecesPerPackage, packaging)}</em></span>; })}</div></td><td><strong className="pt-total-label">{stockLabel}</strong></td><td><div className="pt-readable-trend">
                    {[{ title: 'Vs last month', subtitle: shiftMonth(filters.month, -1), amount: previousSalesChange, percent: trend.vsPrevious }, { title: 'Vs 3-month average', amount: averageSalesChange, percent: trend.vsAverage }].map(comparison => <section className="pt-comparison" key={comparison.title}>
                        <header><strong>{comparison.title}</strong>{comparison.subtitle && <span>{comparison.subtitle}</span>}</header>
                        <div className={`pt-comparison__amount ${comparison.amount > 0 ? 'is-positive' : comparison.amount < 0 ? 'is-negative' : ''}`}>
                            <strong>{comparison.amount === null ? 'Unavailable' : `${comparison.amount > 0 ? '+' : comparison.amount < 0 ? '−' : ''}${money(Math.abs(comparison.amount))}`}</strong>
                            <span>{comparison.amount > 0 ? 'Sales increase' : comparison.amount < 0 ? 'Sales decrease' : 'Sales change'}</span>
                        </div>
                        <div className="pt-comparison__quantity"><span>Quantity change</span><strong className={comparison.percent > 0 ? 'is-positive' : comparison.percent < 0 ? 'is-negative' : ''}>{comparison.percent > 0 ? '+' : ''}{comparison.percent.toFixed(1)}%</strong></div>
                    </section>)}
                    {trend.projection && <section className="pt-forecast">
                        <header>{trend.projection.projected ? 'Month-end forecast' : 'Final output'}</header>
                        <div className="pt-forecast__metrics"><div><span>Sales</span><strong>{money(trend.projection.sales)}</strong></div><div><span>Quantity</span><strong title={soldQuantityTitle(trend.projection.pieces, piecesPerPackage, packaging)}>{projectedLabel}</strong></div></div>
                        {trend.projection.projected && <p>Based on current daily sales</p>}
                        <footer><span className={`pt-forecast__status pt-forecast__status--${trend.assessment.toLowerCase().replace(' ', '-')}`}>{trend.assessment}</span><span><strong>{trend.attainment.toFixed(1)}%</strong> of average quantity</span></footer>
                    </section>}
                </div></td><td className="pt-graph-cell"><IconButton component="a" href={`/productMonthlySalesHistory/product/${item.product_id || item.id}?month=${filters.month}&source=trend`} target="_blank" rel="noopener noreferrer" color="primary" size="small" title={`View comparison graph for ${name}`}><BarChartRoundedIcon fontSize="small"/></IconButton><IconButton component="a" href={`/reports/productTrendComparison/product/${item.product_id || item.id}/customer-impact?${impactParams.toString()}`} target="_blank" rel="noopener noreferrer" color="secondary" size="small" title={`View customer impact for ${name}`}><VisibilityRoundedIcon fontSize="small"/></IconButton></td></tr>;
            })}
