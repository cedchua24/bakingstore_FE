...
import InstallmentPaymentList from "./InstallmentPaymentList";
...
const InstallmentOrder = () => {
    ...
    const validate = (values) => {
        ...
        if (installment.interest == 0) {
            errors.interest = "Amount is Required!";
        }
    ...
    const savePaymentType = () => {
        ...
        if (Object.keys(validate(installment)).length > 0) {
            ...
            setIsAddDisabled(true);
            setErrorStock(true);
            InstallmentPaymentTransactionService.sanctum().then(response => {
                InstallmentPaymentTransactionService.create(installment)
                    .then(response => {
                        setSubmitLoadingAdd(false);
                        setIsAddDisabled(false);
                        setErrorStock(false);
    ...
    const fetchInstallmentDetails = (id) => {
        ...
        InstallmentPaymentTransactionService.fetchInstallmentTransactionByMOP(id)
            .then(response => {
                ...
                >
                    ...
                    <FormControl fullWidth sx={{ m: 1 }} variant="standard">
                        ...
                        <Input
                            ...
                            onChange={onchangeInterestMonthly}
